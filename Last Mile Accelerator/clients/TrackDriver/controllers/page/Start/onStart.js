   ...
   var routes ; 
    ...
    client.data.refreshPackageInfo = function (){
        ...
        http.execute(args,"public.GetPackageInfo",function(response)
        {
            //
            //  At this point "response" is results of the Procedure call
            //
 ...
 function initMap() {
          ...
          routes = directionsService.route(request, function(result, status) {
            if (status == 'OK') {
              directionsRenderer.setDirections(result);
                // var ETA = client.getWidget("ETA");
               // ETA.defaultValue = result.routes[0].legs[0].duration_in_traffic.text;
                console.log(result.routes[0].legs[0].duration_in_traffic.text);
    ...
    client.data.refreshVehicleLocation();
    client.data.refreshPackageInfo();
